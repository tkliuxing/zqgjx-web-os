  ...
  <el-card style="position:relative">
    ...
    <el-table :data="data" border stripe :cell-style="{textAlign:'center'}" :header-cell-style="{textAlign:'center'}" v-loading="loading" element-loading-text='加载中...'
      ...
      <el-table-column label="操作" width="150px">
        <template slot-scope="{row}">
<!--          <el-button title="处理" round type="primary" @click="processTable(row)">处理</el-button>-->
          <el-button title="查看" round type="info"  @click="lookTable(row)" >查看</el-button>
    ...
    <el-dialog :title="form.pk?'编辑':'编辑'" :visible.sync="formDialogVisable" width="50%" :close-on-click-modal="false" :close-on-press-escape="false" top="3vh" center destroy-on-close
      @close="clearForm">
      <LogForm v-model="form" :readonly="readonly" ref="FormComp" @checked="submitForm" />
      <div slot="footer">
        <el-button @click="formDialogVisable=false" icon="el-icon-close">取消</el-button>
        <el-button v-if="!readonly" icon="el-icon-check" type="primary" @click="$refs.FormComp.submit()">确定</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    async loadData() {
      ...
      let res = await getList("formdatareport/5853", params);
      this.data = res.data;
      let res2 = await getList("formdatareport/5854", params);
    ...
    processTable(row) {
      ...
      this.form.make_time = moment(this.form.make_time).format(
        "yyyy-MM-DD HH:mm:ss"
      );
