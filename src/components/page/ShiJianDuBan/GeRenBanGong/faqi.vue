  ...
  <el-card style="position:relative">
    ...
    <el-table :data="data" border stripe :cell-style="{textAlign:'center'}" :header-cell-style="{textAlign:'center'}" v-loading="loading" element-loading-text='加载中...'
      ...
      <el-table-column label="操作" width="200px">
        <template slot-scope="{row}">
          <el-button title="延期" round type="warning"  @click="YanqiTable(row)" v-if="row.isyanqi=='0'">延期</el-button>
          <el-button title="编辑" round type="primary"  @click="editTable(row)" >编辑</el-button>
          <el-popconfirm style="margin-left:10px" confirm-button-text='确定' cancel-button-text='取消' icon="el-icon-info" icon-color="red" title="删除后不可恢复，确定删除吗？" @confirm="delTable(row)">
            <el-button slot="reference" title="删除" round type="danger"  >删除</el-button>
          </el-popconfirm>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 延期处理 -->
    <el-dialog :title="form.pk?'编辑':'新建'" :visible.sync="formDialogVisable1" width="50%" :close-on-click-modal="false" :close-on-press-escape="false" top="3vh" center destroy-on-close
      @close="clearForm">
      <YanqiForm v-model="form" ref="FormComp" @checked="submitForm" />
      <div slot="footer">
        <el-button @click="formDialogVisable1=false" icon="el-icon-close">取消</el-button>
        <el-button icon="el-icon-check" type="primary" @click="$refs.FormComp.submit()">确定</el-button>
      </div>
    </el-dialog>
...
import Form from "../FaBuRenWu/Form.vue";
import YanqiForm from "../FaBuRenWu/YanqiForm.vue";
...
export default {
  ...
  methods: {
    ...
    async loadData() {
      ...
      params["faqiren_id"] = this.userinfo.pk;
    ...
    async submitForm() {
      ...
      try {
        if (
          ...
          this.form.isyanqi = null;
          await patchObj("data", this.form.pk, {
            template_id: "FT204884904327495680",
            isyanqi: this.form.isyanqi,
          });
        }
        if (this.form.pk) {
          ...
          this.form.fuzeren_name = this.userinfo.full_name;
          this.form.fuzeren_id = this.userinfo.pk;
