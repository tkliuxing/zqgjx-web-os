  ...
  <el-card style="position:relative">
    ...
    <div v-if="open_id">
      <div v-if="data.length>0">
        <el-card class="box-card" v-for="item in data" style="margin: -10px">
          <div @click="jilu(item)">
            <el-row>
              <el-col :span="16">
  ...
  methods: {
    ...
    async wx_authorize() {
      ...
      try {
        ...
        this.$message.success("微信授权失败，请重试！！！" + err.response.data.non_field_errors)
      }
    ...
    async getLocalDataSource() {
      ...
      let res7 = await resource.getObjFromList("basetree", {"parent_name": "申请状态", 'org_id': 0});
      this.status_option_data = res7;
    ...
    creatQrCode(data) {
      if (data.status === "同意") {
        let uri = this.url + "/sf_mg?pk=" + data.pk;
        return uri;
      }
      return "";
